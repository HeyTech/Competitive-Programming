...
void day3::update_manhatan_distance_junction(int32_t& junc_x, int32_t& junc_y){
  ...
  //  std::cout << (manhatan_distance_junction > distance_to_junc) << " : " << manhatan_distance_junction << " : " << distance_to_junc << std::endl;
  ...
  if (l1_vertical) {
      if(l1x0 > l2x0 && l1x0 < l2x1){
          if(l2y0 > l1y0 && l2y0 < l1y1){
              // Task 1 prints
              // std::cout << "Intersection found: \t Line 1: " << l1 << " Line 2: " <<  l2 << " -> x: " << l1x0 << " y: " << l2y0 << std::endl;

//              Task 2 prints
              uint32_t l1_dis = l1.p0.distance + std::abs(l2y0);
              uint32_t l2_dis = l2.p0.distance + std::abs(l1x0);
              std::cout << "Vertical -> Line1/distance: " << l1.p0 << " / " << l1.p0.distance << "(" << l1_dis <<")" << " Line2/distance: "<< l2.p0<< " / " << l2.p0.distance  << "(" << l2_dis <<")" << "\t -> x: " << l1x0 << " y: " << l2y0 << "\t\t Tot: "<< l1_dis+l2_dis << std::endl;
      ...
      if(l1y0 > l2y0 && l1y0 < l2y1){
          if(l2x0 > l1x0 && l2x0 < l1x1 ){
              //              std::cout << "Intersection found: \t Line 1: " << l1 << " Line 2: " <<  l2 << " -> x: " << l2x0 << " y: " << l1y0 << std::endl; // Task 1

              uint32_t l1_dis = l1.p0.distance + std::abs(l2x0);
              uint32_t l2_dis = l2.p0.distance + std::abs(l1y0);
              std::cout << "Horizontal -> Line1/distance: " << l1.p0 << " / " << l1.p0.distance << "(" << l1_dis <<")" << " Line2/distance: " <<  l2.p0  << " / " << l2.p0.distance << "(" << l2_dis <<")" << "\t -> x: " << l2x0 << " y: " << l1y0 << "\t\t Tot: "<< l1_dis+l2_dis <<  std::endl; // Task 2
...
void day3::find_junctions(){
  for(std::uint32_t w1{0}; w1 < wire1_lines.size(); w1++){
      for(std::uint32_t w2{0}; w2 < wire2_lines.size(); w2++){
          //          std::cout << wire1_lines[w1] << std::endl;
...
void day3::task1(){
  ...
  find_junctions();
  std::cout << "Manhattan distance from the central port to the closest intersection: " << manhatan_distance_junction << std::endl;
}
